...
// После нажатия "Нужны КП и/или черновой проект" бот ждёт, пока в сделке появится новая задача "Указать рез-т(и)" (id 2774021).
  ...
  const currentQueue =
    previousQueue
      ...
      .then(async () => {
        ...
        // Отменяем уже запланированную отбивку — если она успела бы
        // сработать раньше, чем этот (следующий подряд) пакет файлов
        // закончит загружаться (загрузка теперь ждёт реального
        // завершения на Яндекс.Диске и может занимать больше 1.8 сек),
    ...
    if (imageUrls && imageUrls.length > 0) {
      ...
      const currentQueue = previousQueue
        ...
        .then(async () => {
          ...
          // См. комментарий выше (фото договора) — отменяем уже
